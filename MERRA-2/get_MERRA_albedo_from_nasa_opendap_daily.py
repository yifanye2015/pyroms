...
session = setup_session("yeyifan","&?KPau*5n-JRZX/",url)
...
for month in range(5-1,5):
    ...
    dstart = pyroms_toolbox.date2jday(datetime(year, month+1, 1, 0, 0)) # year, month, day, hr, min 
    roms_time = np.arange(dstart, dstart+daysinmonth[month], 3./24) # x./24 means sampling every x hrs
