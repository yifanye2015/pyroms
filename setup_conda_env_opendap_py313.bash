...
DEFAULT_ENV="pyroms-opendap-py313"
...
if [ ! -f environment_opendap_py313.yaml ]; then
    echo "Error: environment_opendap_py313.yaml not found in the current directory. Please copy one in from the template folders."
    exit 1
fi

echo "Creating conda environment from environment.yaml..."
# Commented out for testing, uncomment when ready
conda env create -n "${ENV_NAME}" -f environment_opendap_py313.yaml || {
    echo "Environment ${ENV_NAME} may already exist. Attempting update..."
    conda env update -n "${ENV_NAME}" -f environment_opendap_py313.yaml
}
