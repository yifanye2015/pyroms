...
url='https://tds.hycom.org/thredds/dodsC/datasets/GLBy0.08/expt_93.0/data/hindcasts/2019/hycom_glby_930_2019010112_t000_ssh.nc'
...
for month in range(5,5+1):
    ...
    for day in range(1,1+1): # range excludes upper bound, if using all days of the month then use range(1,daysinmonth[month]+1)
        ...
        url='http://tds.hycom.org/thredds/dodsC/datasets/GLBy0.08/expt_93.0/data/hindcasts/'+ year_tag +'/hycom_glby_930_' + date_tag + '12_t000_ssh.nc' # Changed URL according to previous url
        #get data from server
        try:
            dataset = netCDF4.Dataset(url)
            var = dataset.variables[invarname][0,2136:2140,1253:1300] # lat,lon; one fewer matrix so remove the second colon
