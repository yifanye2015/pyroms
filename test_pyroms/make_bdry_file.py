...
def do_file(file, src_grd, dst_grd):
    ...
    dst_grd = pyroms.grid.get_ROMS_grid(grid_id, zeta=zeta) # looks for grid id from gridid.txt
...
#    pdb.set_trace()
    ...
    command = ('ncks', '--no-alphabetize', '-O', out_file, bdry_file) 
    subprocess.call(command)
    os.remove(out_file)
    out_file = dst_dir + file.rsplit('/')[-1][:-3] + '_temp_bdry_' + dst_grd.name + '.nc'
    command = ('ncks', '--no-alphabetize', '-A', out_file, bdry_file) 
    subprocess.call(command)
    os.remove(out_file)
