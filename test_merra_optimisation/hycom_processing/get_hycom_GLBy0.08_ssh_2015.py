...
def create_HYCOM_file_ssh(name, time, lon, lat, var): # no z input argument for ssh only
...
url='http://tds.hycom.org/thredds/dodsC/datasets/GLBy0.08/expt_93.0/data/hindcasts/'+ str(year) + '/hycom_glby_930_' + str(year) + '010112_t000_ssh.nc'
...
for date_tag in date_list:
    ...
    url='http://tds.hycom.org/thredds/dodsC/datasets/GLBy0.08/expt_93.0/data/hindcasts/'+ year_tag +'/hycom_glby_930_' + year_tag + date_tag + '12_t000_ssh.nc' # Changed URL according to previous url
    #get data from server
    try:
        dataset = netCDF4.Dataset(url)
        var = dataset.variables[invarname][0,lat_index_lower:lat_index_upper,lon_index_lower:lon_index_upper] # lat,lon; one fewer matrix so remove the second colon
    ...
    create_HYCOM_file_ssh(outfile, jday, lon, lat, var)

print("----------------------")
if len(retry_day) == 0:
    print(f'Sea Surface Height (SSH) files from {year_tag}{start_date} to {year_tag}{end_date} ({n_days} {"day" if n_days == 1 else "days"}) downloaded successfully.')
else:
