...
# Dataset information
MERRA2_M2T1NXRAD_ccid = "C1276812851-GES_DISC" # short name M2T1NXRAD
file_tag = "MERRA2_400.tavg1_2d_rad_Nx."
...
invarnames = ['SWGDN', 'LWGAB', 'CLDTOT', 'ALBEDO']
outvarnames = ['swrad', 'lwrad_down', 'cloud', 'albedo']
outtimenames = ['srf_time', 'lrf_time', 'cloud_time', 'albedo_time']

output_path_rad = "./Forcings/RAD"
output_path_merra_roms = "./Forcings"


if __name__ == '__main__':
    print(f"Starting download for RAD files from {time_range[0]} to {time_range[1]}...")

    # Get urls
    cmr_urls = [urls for urls in get_cmr_urls(ccid=MERRA2_M2T1NXRAD_ccid, time_range=time_range)]
    ...
    Variables = [f"/{invarname}" for invarname in invarnames]
    ...
    dap_to_netcdf(cmr_urls,
                ...
                output_path=output_path_rad)
    
    ds.close()

    print(f"Download completed. Starting processing of RAD files into ROMS format...")

    # Process each file into ROMS format
    rad_filenames = [f for f in os.listdir(output_path_rad) if f.startswith(file_tag) and f.endswith(".nc4")]

    for rad_filename in rad_filenames:
        # Open the RAD file
        rad_filepath = os.path.join(output_path_rad, rad_filename)
        ds_rad = xr.open_dataset(rad_filepath, decode_times=True)
        rad_date = rad_filename[-12:-4] # extract date from filename, format str(YYYYMMDD)
        time_list_3hourly = pd.date_range(start=rad_date, periods=8, freq='3h')
        time_list_daily = pd.date_range(start=rad_date, periods=1, freq='D') # for daily albedo
        ...
        for varname_idx in range(len(invarnames)):
            ...
            ds_var = ds_rad[[invarname]].rename({
            ...
            if outvarname == "albedo":
                var_daily_data = outvar_data.mean(dim=outtimename).values
                var_daily_data = var_daily_data[np.newaxis, :, :] # add time dimension back for creating DataArray
                ...
                var_daily = xr.DataArray(
                    ...
                    coords={
                        outtimename: time_list_daily,
                        'lat': ds_var['lat'],
                        'lon': ds_var['lon']
                    },
                    name=outvarname,
                    attrs={
                        'long_name': ds_var[outvarname].long_name,
                        'units': ds_var[outvarname].units,
                    }
                )

                # Convert back to Dataset
                ds_var = var_daily.to_dataset()

            else: # 3-hourly averaging for other variables
            ...
            # Save file
            outfile = os.path.join(output_path_merra_roms, f"MERRA2_{outvarname}_{'daily' if outvarname == 'albedo' else '3hours'}_{rad_date[:4]}_{rad_date[4:]}.nc")
            ds_var.to_netcdf(outfile, mode="w", unlimited_dims=outtimename)

        ds_rad.close()

    print(f"{', '.join(outvarnames)} 3-hourly (daily for albedo) files created for {time_range[0]} to {time_range[1]}.")

# go work on the bash script to join the daily files tgt into monthly, look at ncrcat