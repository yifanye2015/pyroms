...
# Dataset information
MERRA2_M2T1NXSLV_ccid = "C1276812863-GES_DISC" # short name M2T1NXSLV
file_tag = "MERRA2_400.tavg1_2d_slv_Nx."
...
invarnames = ['SLP', 'QV2M', 'T2M', 'U2M', 'V2M']
outvarnames = ['Pair', 'Qair', 'Tair', 'Uwind', 'Vwind']
outtimenames = ['pair_time', 'qair_time', 'tair_time', 'wind_time', 'wind_time']

output_path_slv = "./Forcings/SLV"
...
if __name__ == '__main__':
    ...
    dap_to_netcdf(cmr_urls,
                ...
                output_path=output_path_slv)
    
    ds.close()

    print(f"Download completed. Starting processing of SLV files into ROMS format...")

    # Process each file into ROMS format
    slv_filenames = [f for f in os.listdir(output_path_slv) if f.startswith(file_tag) and f.endswith(".nc4")]

    for slv_filename in slv_filenames:
        # Open the SLV file
        slv_filepath = os.path.join(output_path_slv, slv_filename)
        ds_slv = xr.open_dataset(slv_filepath, decode_times=True)
        slv_date = slv_filename[-12:-4] # extract date from filename, format str(YYYYMMDD)
        time_list_3hourly = pd.date_range(start=slv_date, periods=8, freq='3h')
        ...
        for varname_idx in range(len(invarnames)):
            ...
            ds_var = ds_slv[[invarname]].rename({
            ...
            # Save file
            outfile = os.path.join(output_path_merra_roms, "MERRA2_" + outvarname + "_3hours_" + slv_date[:4] + "_" + slv_date[4:] + ".nc")
