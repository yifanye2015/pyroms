...
url='http://tds.hycom.org/thredds/dodsC/datasets/GLBy0.08/expt_93.0/data/hindcasts/'+ str(year) + '/hycom_glby_930_' + str(year) + '010112_t000_uv3z.nc'
...
for date_tag in date_list:
    ...
    url='http://tds.hycom.org/thredds/dodsC/datasets/GLBy0.08/expt_93.0/data/hindcasts/'+ year_tag +'/hycom_glby_930_' + year_tag + date_tag + '12_t000_uv3z.nc' # Changed URL according to previous url
...
print("----------------------")
if len(retry_day) == 0:
    print(f'Water_u files from {year_tag}{start_date} to {year_tag}{end_date} ({n_days} {"day" if n_days == 1 else "days"}) downloaded successfully.')
else:
