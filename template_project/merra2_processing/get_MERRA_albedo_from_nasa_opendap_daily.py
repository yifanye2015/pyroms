...
for month in month_range:
    ...
    month_tag = '%02d' %(month)
    outfile = 'Forcings/MERRA2_' + outvarname + '_daily_' + year_tag + '_' + month_tag + '.nc'
    ...
    # Daily albedo data so step = 1 day
    roms_time = np.arange(dstart, dstop + 1, 1) # x./24 means sampling every x hrs; +1 here to include the last day, also avoids errors when crossing a month or year
...
#        file_tag = 'MERRA300'
        ...
        svar = np.array(svar)
        np.seterr(divide='ignore', invalid='ignore') # to ignore warning using missing value for calculations
        var_daily = (svar * mask).sum(axis=0) /  mask.sum(axis=0)
        np.seterr(divide='warn', invalid='warn') # to reactivate warning using missing value for calculations
